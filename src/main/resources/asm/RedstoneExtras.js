...
// add the test: if (block == ModBlocks.BLUESTONE_WIRE || block == ModBlocks.BLUESTONE_PIPE_BLOCK) return false;
function patch_canConnectTo(obj) {
	...
	if (node && node.name == wire) {
		...
		if (node2 && node2.getType() == AbstractInsnNode.LABEL)
		{
			...
			var op3 = new JumpInsnNode(opc.IF_ACMPEQ, op7)
			var op4 = new VarInsnNode(opc.ALOAD, 4)
			var op5 = new FieldInsnNode(opc.GETSTATIC, "com/lupicus/rsx/block/ModBlocks", "BLUESTONE_PIPE_BLOCK", "Lnet/minecraft/block/Block;")
			var op6 = new JumpInsnNode(opc.IF_ACMPNE, op10)
			var op8 = new InsnNode(opc.ICONST_0)
			var op9 = new InsnNode(opc.IRETURN)
